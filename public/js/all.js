        ...
        switch(formId) {
            ...
            case "formCourt":
                ...
                if(!patternAcronym.test(acronymValue)){
                    error += 'Le champ Acronyme ne doit pas être vide et doit avoir entre 1 et 6 caractères.<br>';
                }
...
$( document ).ready(function() {
    ...
    $("#formTeam #name").on("change paste keyup", function() {
        CheckifCanSubmit();
        // Solution who jump out the team name validation because the function TeamAlreadyExsist(data) didn't work in this area (work if you are an user)
        // Remove
        enableButtonValidate();
    });
    ...
    function resetContent(level) {
        ...
        if (level == 3)
        {
            $("#formTeam #teamNew" ).remove();
            $('#formTeam #teamNew').append('<option selected = "selected" disabled = "disabled"  hidden="hidden">Sélectionner</option>'); // append an option tag for the array item
        }
    }
});
